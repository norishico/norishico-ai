...
# 2026-09-20 F9修正: "4"だけ2文字表記("不良")で、他は1文字表記(良/稍/重)だったため不統一
# だった(DB実態は1文字が主流、13,046件 vs 2文字58件。rules/ayokeiba-mc.mdのtrack_cond
# 表記ゆれ問題・障害混入バグの遠因)。1文字表記に統一する。既存データ(2026-07-26新潟の
# 4レース、58件)はUPDATE results SET track_cond='不' WHERE track_cond='不良'で
# 一度だけ修正済み(conn.backup()でバックアップ後に実施)。
BABA_COND = {"1": "良", "2": "稍", "3": "重", "4": "不"}
