...
K_GAP = 0.05              # 車間維持のゲイン(/s)。2026-08-07監査M5対応で凍結
                          # (感度再計測の結果・凍結根拠はTARGET_GAP_Mのコメント参照)
...
# 【2026-08-07監査M5対応: k_gap/target_gap_mは意図的にこの値で凍結する】
# 体系監査で両者が「未較正の隠れペースレバー」(k_gap 0.0125→0.2でS率11→37%等)と
# 指摘されたため、現行較正状態(追い越しペナルティ修正・複数逃げ機構・ダートフェーズ
# 再設計後)でフルevaluate(60セル×n_sim=150)の感度を再計測した(scratchpad/
# gap_params_sweep.py)。結果: k_gap {0.0125,0.1,0.2} / target_gap_m {1.5,6,12} のいずれも
# 事前登録基準(訓練H|e|>0.5pt改善かつ検証改善かつ全ゲートOK)を満たさない
# (最良のk_gap=0.2でも改善は訓練-0.18pt/検証-0.13ptのノイズ水準。tg=12は訓練・検証とも
# 悪化しgate3bバイアス+9.0ptに劣化)。よって較正対象には組み込まず、物理的事前値
# (target_gap_m=3.0m≈1馬身、k_gap=0.05/s)で凍結する。
# 【裏口カップリングの現状(監査指摘の再確認)】target_gap_m>=8.5m相当(車間0.5秒)で
# 死蔵中のgap_easeゲート(follower_ease_s=0.5)が常時発動する構造は現在も残存している
# (tg=12の行が実測: S誤差が訓練9.1→13.7ptに悪化=発動時は過剰なS生成)。tg=3.0では
# 平衡車間0.18秒<0.5秒のため発動せず、凍結によりこの経路は引き続き不活性。
# run_cellはtarget_gap_mをパススルー化済み(params未指定なら本既定値)なので、将来
# 再検証する場合はスイープharnessから直接振れる。
TARGET_GAP_M = 3.0        # P2追走時の目標車間(m、おおよそ1馬身。2026-08-07凍結)
...
# 【2026-08-07同日追記: 東京ゾーン精密化で1600の悪化解消】Wikipedia東京競馬場の芝詳細
# プロファイル(『3コーナー手前に高低差1.5mの上り』)+JRA公式『ダートの起伏構成は芝と
# ほぼ同様』+コース幾何実測(ダート3-4角入口=残り979.9m、ワンターン全距離共通)に基づき、
# 東京ダの上りゾーンを残り1350-1050m(推定)→残り1180-980m(dirt-midrace-20260807bタグ)に
# 更新(実測H率は不使用)。結果: 東京ダ1600はカット対象外(上り入口pos=420>窓400)となり
# 悪化が構造的に解消(42.3→71.3%、実測68.0%)、東京ダ1400も51.0%(誤差26.8→14.5pt)に改善、
# 東京ダ1300(53.3%≒実測53.6%)・京都ダ1200(91.0%)は維持。訓練mean|e|は13.21→10.49pt、
# F=150が引き続き最小(floor再較正不要)。フルゲート: gate1 0.912→0.919/他ゲート不変OK。
DIRT_PHASE_FLOOR_M = 150.0   # 較正済み(2026-08-07、F=150採用。東京ゾーン精密化後も最小を再確認)
