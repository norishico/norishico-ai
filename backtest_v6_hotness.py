...
# ── 血統の旬 (Hotness) ──
# 父種牡馬の直近30日複勝率と過去1年平均の乖離を計算
# cold (-1pt 以下) の馬は買い候補から除外
_hotness_cache = {}

def calc_sire_hotness(sire, race_date, conn):
    """sire の直近30日 vs 過去1年(31-365日前) の複勝率乖離"""
    if not sire:
        return 0
    key = (sire, race_date)
    if key in _hotness_cache:
        return _hotness_cache[key]
    r1 = conn.execute(
    ...
    if not r1 or not r2 or (r1['n'] or 0) < 20 or (r2['n'] or 0) < 100:
        _hotness_cache[key] = 0
        return 0
    recent = r1['t3'] / r1['n'] * 100
    baseline = r2['t3'] / r2['n'] * 100
    dev = recent - baseline
    dev = max(-5, min(5, dev))
    _hotness_cache[key] = dev
    return dev
...
def run_month_v6(conn, sc_conn, year, month):
    ...
    for race in races:
        ...
        # 【血統の旬 Hotness フィルタ】
        # cold sire (-1pt以下) の馬は除外
        hotness = calc_sire_hotness(honmei_sire, race['date'], conn)
        if ev_ok and hotness <= -1:
            ev_ok = False
            buy_zone = None
        # 3勝 normalで hot(+1pt以上)の場合はロック維持(将来challenge併用時の為)
...
if __name__ == '__main__':
    ...
    fname = f'btv6_hotness_{year}.json'
